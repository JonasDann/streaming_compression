// ==============================================================
// Generated by Vitis HLS v2024.1
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2024 Advanced Micro Devices, Inc. All Rights Reserved.
// ==============================================================

`timescale 1 ns / 1 ps 

module gzipcMulticoreStreaming_zlibHuffmanEncoderStatic_18 (
        ap_start,
        start_full_n,
        start_out,
        start_write,
        lz77Stream_5_dout,
        lz77Stream_5_empty_n,
        lz77Stream_5_read,
        huffStream_5_din,
        huffStream_5_full_n,
        huffStream_5_write,
        ap_clk,
        ap_rst,
        ap_done,
        ap_ready,
        ap_idle,
        ap_continue
);


input   ap_start;
input   start_full_n;
output   start_out;
output   start_write;
input  [9:0] lz77Stream_5_dout;
input   lz77Stream_5_empty_n;
output   lz77Stream_5_read;
output  [17:0] huffStream_5_din;
input   huffStream_5_full_n;
output   huffStream_5_write;
input   ap_clk;
input   ap_rst;
output   ap_done;
output   ap_ready;
output   ap_idle;
input   ap_continue;

reg start_write;

reg    real_start;
reg    start_once_reg;
wire    internal_ap_ready;
wire    huffmanProcessingUnit_37_U0_ap_start;
wire    huffmanProcessingUnit_37_U0_ap_done;
wire    huffmanProcessingUnit_37_U0_ap_continue;
wire    huffmanProcessingUnit_37_U0_ap_idle;
wire    huffmanProcessingUnit_37_U0_ap_ready;
wire    huffmanProcessingUnit_37_U0_start_out;
wire    huffmanProcessingUnit_37_U0_start_write;
wire    huffmanProcessingUnit_37_U0_lz77Stream_5_read;
wire   [32:0] huffmanProcessingUnit_37_U0_encodedOutStream_din;
wire    huffmanProcessingUnit_37_U0_encodedOutStream_write;
wire    huffmanEncoderStatic_38_U0_ap_start;
wire    huffmanEncoderStatic_38_U0_ap_done;
wire    huffmanEncoderStatic_38_U0_ap_continue;
wire    huffmanEncoderStatic_38_U0_ap_idle;
wire    huffmanEncoderStatic_38_U0_ap_ready;
wire    huffmanEncoderStatic_38_U0_start_out;
wire    huffmanEncoderStatic_38_U0_start_write;
wire    huffmanEncoderStatic_38_U0_encodedOutStream_read;
wire   [19:0] huffmanEncoderStatic_38_U0_hufCodeStream_din;
wire    huffmanEncoderStatic_38_U0_hufCodeStream_write;
wire    bitPackingStatic_39_U0_ap_start;
wire    bitPackingStatic_39_U0_ap_done;
wire    bitPackingStatic_39_U0_ap_continue;
wire    bitPackingStatic_39_U0_ap_idle;
wire    bitPackingStatic_39_U0_ap_ready;
wire    bitPackingStatic_39_U0_hufCodeStream_read;
wire   [17:0] bitPackingStatic_39_U0_huffStream_5_din;
wire    bitPackingStatic_39_U0_huffStream_5_write;
wire    encodedOutStream_full_n;
wire   [32:0] encodedOutStream_dout;
wire   [2:0] encodedOutStream_num_data_valid;
wire   [2:0] encodedOutStream_fifo_cap;
wire    encodedOutStream_empty_n;
wire    hufCodeStream_full_n;
wire   [19:0] hufCodeStream_dout;
wire   [2:0] hufCodeStream_num_data_valid;
wire   [2:0] hufCodeStream_fifo_cap;
wire    hufCodeStream_empty_n;
wire    ap_sync_ready;
wire   [0:0] start_for_huffmanEncoderStatic_38_U0_din;
wire    start_for_huffmanEncoderStatic_38_U0_full_n;
wire   [0:0] start_for_huffmanEncoderStatic_38_U0_dout;
wire    start_for_huffmanEncoderStatic_38_U0_empty_n;
wire   [0:0] start_for_bitPackingStatic_39_U0_din;
wire    start_for_bitPackingStatic_39_U0_full_n;
wire   [0:0] start_for_bitPackingStatic_39_U0_dout;
wire    start_for_bitPackingStatic_39_U0_empty_n;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 start_once_reg = 1'b0;
end

gzipcMulticoreStreaming_huffmanProcessingUnit_37 huffmanProcessingUnit_37_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(huffmanProcessingUnit_37_U0_ap_start),
    .start_full_n(start_for_huffmanEncoderStatic_38_U0_full_n),
    .ap_done(huffmanProcessingUnit_37_U0_ap_done),
    .ap_continue(huffmanProcessingUnit_37_U0_ap_continue),
    .ap_idle(huffmanProcessingUnit_37_U0_ap_idle),
    .ap_ready(huffmanProcessingUnit_37_U0_ap_ready),
    .start_out(huffmanProcessingUnit_37_U0_start_out),
    .start_write(huffmanProcessingUnit_37_U0_start_write),
    .lz77Stream_5_dout(lz77Stream_5_dout),
    .lz77Stream_5_num_data_valid(3'd0),
    .lz77Stream_5_fifo_cap(3'd0),
    .lz77Stream_5_empty_n(lz77Stream_5_empty_n),
    .lz77Stream_5_read(huffmanProcessingUnit_37_U0_lz77Stream_5_read),
    .encodedOutStream_din(huffmanProcessingUnit_37_U0_encodedOutStream_din),
    .encodedOutStream_num_data_valid(encodedOutStream_num_data_valid),
    .encodedOutStream_fifo_cap(encodedOutStream_fifo_cap),
    .encodedOutStream_full_n(encodedOutStream_full_n),
    .encodedOutStream_write(huffmanProcessingUnit_37_U0_encodedOutStream_write)
);

gzipcMulticoreStreaming_huffmanEncoderStatic_38 huffmanEncoderStatic_38_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(huffmanEncoderStatic_38_U0_ap_start),
    .start_full_n(start_for_bitPackingStatic_39_U0_full_n),
    .ap_done(huffmanEncoderStatic_38_U0_ap_done),
    .ap_continue(huffmanEncoderStatic_38_U0_ap_continue),
    .ap_idle(huffmanEncoderStatic_38_U0_ap_idle),
    .ap_ready(huffmanEncoderStatic_38_U0_ap_ready),
    .start_out(huffmanEncoderStatic_38_U0_start_out),
    .start_write(huffmanEncoderStatic_38_U0_start_write),
    .encodedOutStream_dout(encodedOutStream_dout),
    .encodedOutStream_num_data_valid(encodedOutStream_num_data_valid),
    .encodedOutStream_fifo_cap(encodedOutStream_fifo_cap),
    .encodedOutStream_empty_n(encodedOutStream_empty_n),
    .encodedOutStream_read(huffmanEncoderStatic_38_U0_encodedOutStream_read),
    .hufCodeStream_din(huffmanEncoderStatic_38_U0_hufCodeStream_din),
    .hufCodeStream_num_data_valid(hufCodeStream_num_data_valid),
    .hufCodeStream_fifo_cap(hufCodeStream_fifo_cap),
    .hufCodeStream_full_n(hufCodeStream_full_n),
    .hufCodeStream_write(huffmanEncoderStatic_38_U0_hufCodeStream_write)
);

gzipcMulticoreStreaming_bitPackingStatic_39 bitPackingStatic_39_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(bitPackingStatic_39_U0_ap_start),
    .ap_done(bitPackingStatic_39_U0_ap_done),
    .ap_continue(bitPackingStatic_39_U0_ap_continue),
    .ap_idle(bitPackingStatic_39_U0_ap_idle),
    .ap_ready(bitPackingStatic_39_U0_ap_ready),
    .hufCodeStream_dout(hufCodeStream_dout),
    .hufCodeStream_num_data_valid(hufCodeStream_num_data_valid),
    .hufCodeStream_fifo_cap(hufCodeStream_fifo_cap),
    .hufCodeStream_empty_n(hufCodeStream_empty_n),
    .hufCodeStream_read(bitPackingStatic_39_U0_hufCodeStream_read),
    .huffStream_5_din(bitPackingStatic_39_U0_huffStream_5_din),
    .huffStream_5_num_data_valid(3'd0),
    .huffStream_5_fifo_cap(3'd0),
    .huffStream_5_full_n(huffStream_5_full_n),
    .huffStream_5_write(bitPackingStatic_39_U0_huffStream_5_write)
);

gzipcMulticoreStreaming_fifo_w33_d4_S_x11 encodedOutStream_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(huffmanProcessingUnit_37_U0_encodedOutStream_din),
    .if_full_n(encodedOutStream_full_n),
    .if_write(huffmanProcessingUnit_37_U0_encodedOutStream_write),
    .if_dout(encodedOutStream_dout),
    .if_num_data_valid(encodedOutStream_num_data_valid),
    .if_fifo_cap(encodedOutStream_fifo_cap),
    .if_empty_n(encodedOutStream_empty_n),
    .if_read(huffmanEncoderStatic_38_U0_encodedOutStream_read)
);

gzipcMulticoreStreaming_fifo_w20_d4_S_x3 hufCodeStream_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(huffmanEncoderStatic_38_U0_hufCodeStream_din),
    .if_full_n(hufCodeStream_full_n),
    .if_write(huffmanEncoderStatic_38_U0_hufCodeStream_write),
    .if_dout(hufCodeStream_dout),
    .if_num_data_valid(hufCodeStream_num_data_valid),
    .if_fifo_cap(hufCodeStream_fifo_cap),
    .if_empty_n(hufCodeStream_empty_n),
    .if_read(bitPackingStatic_39_U0_hufCodeStream_read)
);

gzipcMulticoreStreaming_start_for_huffmanEncoderStatic_38_U0 start_for_huffmanEncoderStatic_38_U0_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(start_for_huffmanEncoderStatic_38_U0_din),
    .if_full_n(start_for_huffmanEncoderStatic_38_U0_full_n),
    .if_write(huffmanProcessingUnit_37_U0_start_write),
    .if_dout(start_for_huffmanEncoderStatic_38_U0_dout),
    .if_empty_n(start_for_huffmanEncoderStatic_38_U0_empty_n),
    .if_read(huffmanEncoderStatic_38_U0_ap_ready)
);

gzipcMulticoreStreaming_start_for_bitPackingStatic_39_U0 start_for_bitPackingStatic_39_U0_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(start_for_bitPackingStatic_39_U0_din),
    .if_full_n(start_for_bitPackingStatic_39_U0_full_n),
    .if_write(huffmanEncoderStatic_38_U0_start_write),
    .if_dout(start_for_bitPackingStatic_39_U0_dout),
    .if_empty_n(start_for_bitPackingStatic_39_U0_empty_n),
    .if_read(bitPackingStatic_39_U0_ap_ready)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        start_once_reg <= 1'b0;
    end else begin
        if (((real_start == 1'b1) & (internal_ap_ready == 1'b0))) begin
            start_once_reg <= 1'b1;
        end else if ((internal_ap_ready == 1'b1)) begin
            start_once_reg <= 1'b0;
        end
    end
end

always @ (*) begin
    if (((start_once_reg == 1'b0) & (start_full_n == 1'b0))) begin
        real_start = 1'b0;
    end else begin
        real_start = ap_start;
    end
end

always @ (*) begin
    if (((start_once_reg == 1'b0) & (real_start == 1'b1))) begin
        start_write = 1'b1;
    end else begin
        start_write = 1'b0;
    end
end

assign ap_done = bitPackingStatic_39_U0_ap_done;

assign ap_idle = (huffmanProcessingUnit_37_U0_ap_idle & huffmanEncoderStatic_38_U0_ap_idle & bitPackingStatic_39_U0_ap_idle);

assign ap_ready = huffmanProcessingUnit_37_U0_ap_ready;

assign ap_sync_ready = huffmanProcessingUnit_37_U0_ap_ready;

assign bitPackingStatic_39_U0_ap_continue = ap_continue;

assign bitPackingStatic_39_U0_ap_start = start_for_bitPackingStatic_39_U0_empty_n;

assign huffStream_5_din = bitPackingStatic_39_U0_huffStream_5_din;

assign huffStream_5_write = bitPackingStatic_39_U0_huffStream_5_write;

assign huffmanEncoderStatic_38_U0_ap_continue = 1'b1;

assign huffmanEncoderStatic_38_U0_ap_start = start_for_huffmanEncoderStatic_38_U0_empty_n;

assign huffmanProcessingUnit_37_U0_ap_continue = 1'b1;

assign huffmanProcessingUnit_37_U0_ap_start = real_start;

assign internal_ap_ready = ap_sync_ready;

assign lz77Stream_5_read = huffmanProcessingUnit_37_U0_lz77Stream_5_read;

assign start_for_bitPackingStatic_39_U0_din = 1'b1;

assign start_for_huffmanEncoderStatic_38_U0_din = 1'b1;

assign start_out = real_start;

endmodule //gzipcMulticoreStreaming_zlibHuffmanEncoderStatic_18
